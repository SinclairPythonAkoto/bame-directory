...
def search():
    if request.method == 'POST':
        ...
        if selectCategory == "arts":
            results = db.child("/Bame_Business/business/arts_Media_Tech").get().val()
            if search in results['userName'] or search in results['businessName']:
                return render_template('new.html', results=results)
            return "sorry couldn't find what you were looking for!"

        if selectCategory == "foods":
            results = db.child("/Bame_Business/business/foods_Restaurants_Takeaways").get().val()
            if search in results['userName'] or search in results['businessName']:
                return render_template('new.html', results=results)
        #     return "sorry couldn't find what you were looking for!"
    else:
